...
- **`README.md`** — fill the project name, *What it does* (human-facing, bullet-first), *Getting started* (clone → `powershell.exe -ExecutionPolicy Bypass -File .\scripts\windows\setup-dev.ps1` → the dev commands), *Development* (the per-command summary).
...
- **`scripts/windows/setup-dev.ps1`** — append the stack's setup below the `# --- stack-specific ---` marker: runtime/toolchain install + version checks + dependency install (e.g. `node`, `npm ci`). Match the existing `Have`/`Missing`/`Manual` conventions so the summary still reports cleanly. Keep privileged/interactive steps as manual entries, not auto-run.
...
Run `powershell.exe -ExecutionPolicy Bypass -File .\scripts\windows\setup-dev.ps1` in-session. Tool checks and Bitbucket secret setup may surface as manual entries. Relay the result plainly:
