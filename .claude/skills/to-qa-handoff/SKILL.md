...
- Per-story `testing.md` files exist at `docs/ai-runs/{slug}/{story-id}/testing.md` for each `done` story. If any are missing, the cleanup script likely hasn't run yet — refuse and tell the human to run `scripts/cleanup-worktrees.sh` first.
...
- **Refuse on incomplete inputs.** Pre-`done` story → refuse. Missing `testing.md` files in the integration tree → refuse and point to `scripts/cleanup-worktrees.sh`.
