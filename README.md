...
1. Clone this repo.
2. Run `bash .codex/scripts/setup-dev.sh` to verify the AISDLC harness tools and global monorepo toolchain.
3. Copy the needed `.codex/` bundle and `AGENTS.md` into the Corpay monorepo worktree for active Deals feature work.
...
- `bash .codex/scripts/setup-dev.sh` - verifies baseline AISDLC tooling plus global `codex`, `dotnet`, `node`, and `npm` availability.
- `bash .codex/scripts/run-codex-loop.sh` - runs the AISDLC story implementation loop from an integration branch.
- `bash .codex/scripts/cleanup-codex-worktrees.sh` - copies back run artifacts and removes completed story worktrees.
...
- Automation scripts: `.codex/scripts/run-codex-loop.sh` and `.codex/scripts/cleanup-codex-worktrees.sh`.

## Documentation

- `.codex/CONTEXT.md` - Deals glossary and domain terms.
- `.codex/skills/README.md` - AISDLC workflow and skill catalog.
- `.codex/knowledge/` - architecture notes and ADRs.

## Client repo ignore

When this bundle is copied into the Corpay monorepo for active feature work, ignore the portable Codex assets there:
